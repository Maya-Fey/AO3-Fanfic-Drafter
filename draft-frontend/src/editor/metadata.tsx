...
const RatingSelector: (props: RatingSelectorProps)=>JSX.Element =  (props: RatingSelectorProps)=>{
    return (<React.Fragment>
        {[Rating.NOT_RATED, Rating.GENERAL_AUDIENCES, Rating.TEEN, Rating.MATURE, Rating.EXPLICIT]
            .map(rating=>[
                <input key={String(rating) + "input"} type="radio" name="rating" id={String(rating)} value={String(rating)} defaultChecked={props.fic.meta.rating == rating} onChange={action(()=>{props.fic.meta.rating = rating})}/>,
                <label key={String(rating) + "label"} htmlFor={String(rating)}>{String(rating)}</label>])
